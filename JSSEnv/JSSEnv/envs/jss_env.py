...
class JssEnv(gym.Env):
    ...
    def __init__(self, env_config: Optional[Dict[str, Any]] = None):
        ...
        # used for plotting - couleurs distinctes pour chaque machine
        self.colors = {
            'Machine 0': 'rgb(138, 43, 226)',   # Violet (CNC)
            'Machine 1': 'rgb(30, 144, 255)',   # Bleu (Fraiseuse)
            'Machine 2': 'rgb(255, 140, 0)',    # Orange (Tour)
            'Machine 3': 'rgb(50, 205, 50)',    # Vert clair (Perceuse)
            'Machine 4': 'rgb(220, 20, 60)',    # Rouge (Polisseuse)
        }
        """
    ...
    def reset(self, seed: Optional[int] = None, options: Optional[dict] = None) -> Tuple[Dict[str, np.ndarray], dict]:
        ...
        Returns:
            Tuple of (observation, info) where:
                - observation: Dictionary with keys 'real_obs' and 'action_mask'
                - info: Empty dictionary (for gymnasium API compatibility)
        """
        if seed is not None:
            np.random.seed(seed)
            random.seed(seed)
    ...
    def step(self, action: int) -> Tuple[Dict[str, np.ndarray], float, bool, bool, Dict]:
        ...
        if action == self.jobs:  # No-op action
            ...
            # Safety check: if job has completed all operations, treat as no-op
            if current_time_step_job >= self.machines:
                # Job already completed, return current state without changes
                return (
                    self._get_current_state_representation(),
                    0.0,
                    self._is_done(),
    ...
    def increase_time_step(self) -> int:
        ...
        # Safety check: if no more time steps, return 0
    ...
    def render(self, mode: str = "human") -> Optional[Figure]:
        ...
        # Détecter le type de pièce basé sur la séquence des machines
        def detect_part_type(job_id):
            """Détecte le type de pièce (A, B, C) basé sur la séquence des machines."""
            first_machine = self.instance_matrix[job_id][0][0]
            second_machine = self.instance_matrix[job_id][1][0]
            
            # Part A: CNC (0) -> Fraiseuse (1)
            # Part B: Tour (2) -> CNC (0)
            # Part C: Fraiseuse (1) -> Perceuse (3)
        ...
        for job in range(self.jobs):
            ...
            while i < self.machines and self.solution[job][i] != -1:
                dict_op = dict()
                # Ajouter le type de pièce dans le label
                dict_op["Task"] = f"Job {job} (Pièce {part_type})"
                # Utiliser des minutes simples au lieu de timestamps
                start_min = self.solution[job][i]
                finish_min = start_min + self.instance_matrix[job][i][1]
                dict_op["Start"] = start_min
                dict_op["Finish"] = finish_min
        ...
        if len(df) > 0:
            ...
            # Créer le Gantt avec Plotly Express pour un meilleur contrôle
            import plotly.express as px
            import plotly.graph_objects as go
            
            # Créer le figure manuellement pour avoir un contrôle total
            fig = go.Figure()
            
            # Créer la liste complète de tous les jobs avec leur type de pièce
            all_jobs = []
            for job in range(self.jobs):
                part_type = detect_part_type(job)
                all_jobs.append(f"Job {job} (Pièce {part_type})")
            
            # Ajouter une trace invisible pour chaque job pour forcer l'affichage de tous les jobs
            for job_label in all_jobs:
            ...
            # Ajouter les barres par machine
            for machine_name, color in self.colors.items():
                machine_df = df[df['Resource'] == machine_name]
                ...
                for _, row in machine_df.iterrows():
                    fig.add_trace(go.Bar(
                        ...
                        base=row['Start'],
                        text=f"{machine_name}<br>{row['Start']:.0f}-{row['Finish']:.0f} min",
                        textposition='inside',
                        hovertemplate=f"<b>{row['Task']}</b><br>{machine_name}<br>Début: {row['Start']:.0f} min<br>Fin: {row['Finish']:.0f} min<br>Durée: {row['Finish']-row['Start']:.0f} min<extra></extra>",
                        showlegend=False if machine_df.index[0] != row.name else True
                    ))
            
            # Configurer l'axe X en minutes
            fig.update_xaxes(
                title_text="Temps (minutes)",
                range=[0, self.current_time_step * 1.05]
            )
            
            # Inverser l'axe Y pour avoir Job 0 en haut et forcer l'affichage de tous les jobs
            fig.update_yaxes(
                title_text="Jobs",
                categoryorder='array',
                categoryarray=all_jobs[::-1]  # Inverser pour avoir Job 0 en haut
            )
            
            fig.update_layout(
                title=f"Planning Atelier - Makespan: {self.current_time_step} min",
                height=max(400, self.jobs * 20),  # Hauteur dynamique selon le nombre de jobs
                showlegend=True,
                hovermode='closest',
                barmode='overlay',
                legend=dict(
